...
RSpec.describe Milight::V6::Bridge do
  let(:command) { double(Milight::V6::Command) }

  subject { Milight::V6::Bridge.new(command) }

  describe "#on" do
    it "sends a command to switch lights on" do
      expect(command).to receive(:execute).with(1, [0x31, 0x00, 0x00, 0x00, 0x03, 0x03, 0x00, 0x00, 0x00])
      subject.on
    end
  ...
  describe "#off" do
    it "sends a command to switch lights off" do
      expect(command).to receive(:execute).with(1, [0x31, 0x00, 0x00, 0x00, 0x03, 0x04, 0x00, 0x00, 0x00])
      subject.off
    end
  ...
  describe "#brightness" do
    it "sends a command to change brightness" do
      expect(command).to receive(:execute).with(1, [0x31, 0x00, 0x00, 0x00, 0x02, 0x32, 0x00, 0x00, 0x00])
  ...
  describe "#white_light" do
    it "sends a command to change color temperature to white light" do
      expect(command).to receive(:execute).with(1, [0x31, 0x00, 0x00, 0x00, 0x03, 0x05, 0x00, 0x00, 0x00])
      subject.white_light
    end
  ...
  describe "#hue" do
    it "sends a command to change hue" do
      expect(command).to receive(:execute).with(1, [0x31, 0x00, 0x00, 0x00, 0x01, 0xFF, 0xFF, 0xFF, 0xFF])
      subject.hue(255)
    end
